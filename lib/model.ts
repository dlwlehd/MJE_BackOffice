...
export type DemoData = {version:1; revision:number; events:AcademicEvent[]; subjects:Subject[]; makeups:Makeup[]; consultations:Consultation[]; archives:Archive[]; audit:Audit[]; teacherSchedules:Record<string,string[]>; teacherRotations:TeacherRotation[]; teachers:string[]; teacherOverrides:Record<string,string>};
...
export function makeSeed():DemoData{
 ...
 return {version:1,revision:0,events,subjects,makeups,consultations,archives:[],audit:[],teachers:[...staffNames],teacherOverrides:{},teacherSchedules:{},teacherRotations:[{startDate:sat,teachers:[...staffNames]}]};
}


// Rules apply from their start date; later rules preserve earlier weekly assignments.
export function dutyTeacher(data:Pick<DemoData,'teacherRotations'> & Partial<Pick<DemoData,'teacherOverrides'>>,date:string):string{
 if(!validDate(date)||dayDate(date).getDay()!==6)return '';
 if(data.teacherOverrides?.[date])return data.teacherOverrides[date];
...
export function migrateDemo(value:DemoData):DemoData{
 value={...value,teachers:value.teachers??[...staffNames],teacherOverrides:value.teacherOverrides??{}};
...
export function validateTeacherOrder(date:string,teachers:string[],registered=staffNames){
 ...
 if(new Set(teachers).size!==teachers.length||teachers.some(n=>!registered.includes(n)))throw Error('담당 선생님을 중복 없이 선택해 주세요.');
}
...
export function registerTeacher(data:DemoData,name:string):DemoData{
 const clean=name.trim();
 if(!clean||clean.length>40)throw Error('선생님 이름을 1~40자로 입력해 주세요.');
 if(data.teachers.some(n=>n.normalize('NFC')===clean.normalize('NFC')))throw Error('이미 등록된 이름입니다. 동명이인은 구분할 이름을 입력해 주세요.');
 return {...data,teachers:[...data.teachers,clean]};
}
export function setDutyOverride(data:DemoData,date:string,name:string):DemoData{
 if(!validDate(date)||dayDate(date).getDay()!==6)throw Error('토요일을 선택해 주세요.');
 if(name&&!data.teachers.includes(name))throw Error('등록된 선생님을 선택해 주세요.');
 const teacherOverrides={...data.teacherOverrides};
 if(name)teacherOverrides[date]=name;else delete teacherOverrides[date];
 return {...data,teacherOverrides};
}
