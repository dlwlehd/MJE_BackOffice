...
export type DemoData = {version:1; revision:number; events:AcademicEvent[]; subjects:Subject[]; makeups:Makeup[]; consultations:Consultation[]; archives:Archive[]; audit:Audit[]; teacherSchedules:Record<string,string[]>};
...
export function validateConsultation(c:Consultation,all:Consultation[]){if(!c.subjectId||!c.target.trim())throw Error('관련 학생과 상담 대상자를 입력해 주세요.');if(!validDate(c.date)||!c.start||!c.end||c.start>=c.end)throw Error('상담 날짜와 시작·종료 시각을 확인해 주세요.');if(['취소','미방문'].includes(c.status)&&!c.reason.trim())throw Error('취소 또는 미방문 사유를 남겨 주세요.');if(c.status==='완료'&&!c.summary.trim())throw Error('기록 완료 전 결과 요약을 입력해 주세요.');if(c.status!=='취소'&&all.some(x=>x.id!==c.id&&x.status!=='취소'&&x.staff===c.staff&&x.date===c.date&&overlaps(x.start,x.end,c.start,c.end)))throw Error('같은 상담자의 예약 시간이 겹칩니다. 다른 시간으로 선택해 주세요.');}
export function blankEvent(date=today()):AcademicEvent{return {id:uid(),title:'',category:'event',start:date,end:date,allDay:true,startTime:'14:00',endTime:'15:00',school:'',audience:'',staff:staffNames[0],location:'',note:'',tentative:false};}
...
export function makeSeed():DemoData{
 ...
 const makeups:Makeup[]=subjects.slice(0,6).map((s,i)=>({...blankMakeup(sat),id:'m'+i,studentId:s.id,start:i<3?'10:00':'11:00',end:i===0?'12:30':i<3?'12:00':'13:00',staff:staffNames[i%3],kind:i===1?'정기':'보강',attendance:i>1?'출석':'미체크',task:['독해 오답 정리','문법 4단원 복습','단어 테스트'][i%3],completed:i===5,result:i===5?'오답 정리와 단어 테스트를 완료했습니다.':'',note:i===0?'독해 지문 2개 추가 연습':''}));
 ...
 return {version:1,revision:0,events,subjects,makeups,consultations,archives:[],audit:[],teacherSchedules:{[sat]:[...staffNames]}};
}


// Explicit daily orders take precedence, including an intentionally empty assignment.
export function teacherOrder(data:DemoData,date:string):string[]{
 if(Object.prototype.hasOwnProperty.call(data.teacherSchedules,date))return [...data.teacherSchedules[date]];
 const assigned=new Set(data.makeups.filter(m=>m.date===date).map(m=>m.staff));
 return [...staffNames.filter(n=>assigned.has(n)),...Array.from(assigned).filter(n=>!staffNames.includes(n))];
}
