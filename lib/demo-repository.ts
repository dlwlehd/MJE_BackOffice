...
export function loadDemo():DemoData {const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return makeSeed();const value=JSON.parse(raw);if(value.version!==1||!['events','subjects','makeups','consultations','archives','audit'].every(k=>Array.isArray(value[k])))throw Error('저장된 데모 데이터를 읽을 수 없습니다. 데모 초기화로 다시 시작할 수 있습니다.');return {...value,teacherSchedules:value.teacherSchedules??{}};}
